...
from csp import MinesweeperAI as CSPAI
from pm import MinesweeperAI as PMAI
import os
...
HEIGHT, WIDTH, MINES = DIFFICULTIES[selected_difficulty]
...
ai = CSPAI(height=HEIGHT, width=WIDTH) if selected_ai == "CSP" else PMAI(height=HEIGHT, width=WIDTH)
...
def reset_game_state():
    global game, ai, revealed, flags, lost, mine_detonated
    game = Minesweeper(height=HEIGHT, width=WIDTH, mines=MINES)
    if selected_ai == "CSP":
        ai = CSPAI(height=HEIGHT, width=WIDTH)
    else:
        ai = PMAI(height=HEIGHT, width=WIDTH)
    revealed, flags = set(), set()
    lost, mine_detonated = False, None
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if instructions:
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                mouse = event.pos
                if cspButton.collidepoint(mouse):
                    selected_ai = "CSP"
                    ai = CSPAI(height=HEIGHT, width=WIDTH)
                    instructions = False
                elif pmButton.collidepoint(mouse):
                    selected_ai = "PM"
                    ai = PMAI(height=HEIGHT, width=WIDTH)
                    instructions = False
        if instructions and event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            mouse = event.pos
            # Toggle dropdown
            if dropdownRect.collidepoint(mouse):
                dropdown_open = not dropdown_open
            elif dropdown_open:
                # Check if clicked one of the options
                for idx, diff in enumerate(DIFFICULTIES.keys()):
                    optionRect = pygame.Rect(dropdownRect.x, dropdownRect.y + (idx+1) * 45, dropdownRect.width, 40)
                    if optionRect.collidepoint(mouse):
    ...
    if instructions:
        ...
        # Play with CSP button
        cspButton = pygame.Rect((width / 4), (3 / 4) * height - 60, (width / 2), 50)
        cspText = mediumFont.render("Play with CSP", True, BLACK)
        cspTextRect = cspText.get_rect()
        cspTextRect.center = cspButton.center
        pygame.draw.rect(screen, WHITE, cspButton)
        screen.blit(cspText, cspTextRect)

        # Play with PM button
        pmButton = pygame.Rect((width / 4), (3 / 4) * height + 10, (width / 2), 50)
        pmText = mediumFont.render("Play with PM", True, BLACK)
        pmTextRect = pmText.get_rect()
        pmTextRect.center = pmButton.center
        pygame.draw.rect(screen, WHITE, pmButton)
        screen.blit(pmText, pmTextRect)

        # Dropdown menu for difficulty
        dropdownRect = pygame.Rect(width / 4, (3 / 4) * height - 200, width / 2, 50)
        pygame.draw.rect(screen, WHITE, dropdownRect)

        # Main dropdown box
        pygame.draw.rect(screen, (220,220,220), dropdownRect)  # light grey
        pygame.draw.rect(screen, BLACK, dropdownRect, 2)       # black border

        # ▼ arrow
        pygame.draw.polygon(screen, BLACK, [
            (dropdownRect.right - 20, dropdownRect.centery - 5),
            (dropdownRect.right - 10, dropdownRect.centery - 5),
            (dropdownRect.right - 15, dropdownRect.centery + 5)
        ])

        # Current text
        difficultyText = mediumFont.render(selected_difficulty, True, BLACK)
        screen.blit(difficultyText, (dropdownRect.x + 10, dropdownRect.y + 10))

        # Expanded list (if open)
        if dropdown_open:
            for idx, diff in enumerate(DIFFICULTIES.keys()):
                optionRect = pygame.Rect(dropdownRect.x, dropdownRect.y + (idx+1)*45, dropdownRect.width, 50)
                pygame.draw.rect(screen, (245,245,245), optionRect)   # off-white
                pygame.draw.rect(screen, BLACK, optionRect, 1)        # border
                optionText = mediumFont.render(diff, True, BLACK)
                screen.blit(optionText, (optionRect.x + 10, optionRect.y + 10))



        # Check if AI selection button clicked
    ...
    if autoplay or makeAiMove:
        ...
        # Use next_move() if AI has it (PM AI), else fallback to safe/random (CSP AI)
        if hasattr(ai, "next_move"):
            move = ai.next_move()
        else:
